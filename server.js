  ...
  think(input) {
    ...
    if (input.target) {
      ...
      if (this.timer <= 0 || util.getDistance(this.body, this.goal) < this.body.SIZE || this.state === 0) {
        this.timer = Math.random() * 500 | 0;
        this.state = 1;this.goal = room.randomType(Math.random() > .9 ? "bas1" : "norm" ? "nest" : "bas1");
      }
    }
    return {
      goal: this.goal
...
class io_wanderT2 extends IO {
  ...
  think(input) {
    ...
    if (input.target) {
      ...
      if (this.timer <= 0 || util.getDistance(this.body, this.goal) < this.body.SIZE || this.state === 0) {
        this.timer = Math.random() * 500 | 0;
        this.state = 1;this.goal = room.randomType(Math.random() > .9 ? "bas2" : "norm" ? "nest" : "bas2");
      }
    }
    return {
      goal: this.goal
...
class io_wanderT3 extends IO {
  constructor(body) {
    super(body);
    this.goal = (room.randomType('bas3'));
    this.timer = Math.random() * 500 | 0;
    this.defendTick = -1;
    this.state = 1;
  }
  think(input) {
    ...
    if (input.target) {
      ...
      if (this.timer <= 0 || util.getDistance(this.body, this.goal) < this.body.SIZE || this.state === 0) {
        this.timer = Math.random() * 500 | 0;
        this.state = 1;this.goal = room.randomType(Math.random() > .9 ? "bas3" : "norm" ? "nest" : "bas3");
      }
    }
    return {
      goal: this.goal
...
class io_wanderT4 extends IO {
  constructor(body) {
    super(body);
    this.goal = (room.randomType('bas4'));
    this.timer = Math.random() * 500 | 0;
    this.defendTick = -1;
    this.state = 1;
  }
  think(input) {
    ...
    if (input.target) {
      ...
      if (this.timer <= 0 || util.getDistance(this.body, this.goal) < this.body.SIZE || this.state === 0) {
        this.timer = Math.random() * 500 | 0;
        this.state = 1;this.goal = room.randomType(Math.random() > .9 ? "bas4" : "norm" ? "nest" : "bas4");
      }
    }
    return {
      goal: this.goal
...
class Entity {
    ...
    define(set) {
        ...
        if (set.ACCETS_FOOD_SCORE != null) {
          this.settings.acceptFoodScore = set.ACCEPTS_FOOD_SCORE
        }
        ...
        if (set.TRUE_SIZE != null) { 
            this.squiggle = set.TRUE_SIZE; 
        }
        if (set.SPAWN != null) this.spawn = set.SPAWN
        ...
        if (set.TRUE_SCORE != null) {
          this.trueScore = Math.max(this.trueScore, set.TRUE_SCORE * this.squiggle)
        }
    ...
    refreshBodyAttributes() {
        ...
        this.damage = this.DAMAGE * this.skill.atk + 0.006 * this.skill.level;

        this.penetration = this.PENETRATION + 1.5 * (this.skill.brst + 0.8 * (this.skill.atk - 1)) + 0.003 * this.skill.level;
        ...
        this.pushability = this.PUSHABILITY - 0.003 * this.skill.level;        
    }    
    ...
    get size() {
        if (this.bond == null) return (this.coreSize || this.SIZE) * (1 + (this.skill.level*0.02222222222)*1.1);
    ...
    contemplationOfMortality() {
        ...
        if (this.isDead()) {
            if (this.spawn) {
              switch (this.spawn) {
                case 'lieutenantSoldier': {
                  let o = new Entity({
                    x: this.x,
                    y: this.y
                  })
                  o.team = -100
                  switch (ran.chooseChance(1, 0.25)) {
                    case 0: {
                      o.define(Class.lieutenantSoldier)
                    } break;
                    case 1: {
                      o.define(Class.lieutenantEliteSoldier)
                    } break;
                  }
                }; break
                case 'lieutenantLeader': {
                  let o = new Entity({
                    x: this.x,
                    y: this.y
                  })
                  o.team = -100
                  o.define(Class.lieutenant)
                }; break
                case "summonStorm": {
                  let o = new Entity({
                    x: this.x,
    ...
    return {
        ...
        connect: (() => {
            ...
            function incoming(message, socket) {
                ...
                case 'A': { // testbed cheat
                    if (m.length !== 0) { socket.kick('Ill-sized neutral request.'); return 1; }
                    // cheatingbois
                    if (player.body != null) { if (socket.key === process.env.SECRET) {
                        let o = new Entity({
                          x: player.body.x + player.body.control.target.x,
                          y: player.body.y + player.body.control.target.y
                        })
                        o.define(Class.lieutenantSummonMain)
                    } }
                } break; 
    ...
    let collide = (() => {
        ...
        return collision => {
            ...
            if (instance.settings.hitsOwnType === other.settings.hitsOwnType) {
                ...
                case 'tank': if (instance.type === "tank" && other.type === "tank" || other.type === "tank" && instance.type === "tank") firmcollide(instance, other); break;
                }
            }     
        };
    })();
...
var maintainloop = (() => {
      ...
      if (timer == 30) {
        ...
        for (let i = 0; i < 10; i++) {
          if (i <= 6) {
            switch (wave) {
              case i:
                choice = [[Class.elite_sprayer, Class.elite_destroyer, Class.elite_gunner], 1 + (i * .4), 'a', 'nest'];  
                break;
            }
          } else if (i >= 7 & i <= 9) {
              switch (wave) {
                case i:
                  choice = [[Class.legionaryCrasher, Class.zaphkiel, Class.elite_destroyer, Class.elite_sprayer], 1 + (i * 0.4), 'a', 'nest'];
    ...
    let makenpcs = (() => {
        // Make base protectors if needed.
            /*let f = (loc, team) => { 
               setInterval(() => { let o = new Entity(loc);
                    o.define(Class.bot);
                    o.define(Class.twin);
                    o.skill.set([15, 15, 15, 15, 15, 15, 15, 15, 15, 15])
                    o.team = -team;
        ...
        return () => {
            ...
            // Bots
                ...
                bots.forEach(o => {
                    ...
                    o.color = 10
                    o.skill.score = 26302
                    o.skill.set(Array.from({ length: 10 }, () => Math.floor(Math.random() * 10)))
                    o.team = -1
                    o.controllers.push(new io_wanderT1(o))
                    o.refreshBodyAttributes()
                    tanks1.push(o)
                  }
                
                  tanks1 = tanks1.filter(e => { return !e.isDead(); });
                  tanks1.forEach(o => {
                    if (o.skill.level < 45) {
                      o.skill.maintain();
                    }
          ...
          if (tanks2.length < c.TANKST2) {
                    let o = new Entity(room.randomType("bas2"))
                    o.define(Class.bot)
                    o.define(Class.basic)
                    o.name += ran.chooseBotName()
                    o.color = 11
                    ...
                    o.skill.set(Array.from({ length: 10 }, () => Math.floor(Math.random() * 10)))
                    o.team = -2
                    o.controllers.push(new io_wanderT2(o))
                    o.refreshBodyAttributes()
                    tanks2.push(o)
                  }
                
                  tanks2 = tanks2.filter(e => { return !e.isDead(); });
                  tanks2.forEach(o => {
                    if (o.skill.level < 45) {
                      o.skill.maintain();
                    }
          ...
          if (tanks3.length < c.TANKST3) {
                    let o = new Entity(room.randomType("bas3"))
                    o.define(Class.bot)
                    o.define(Class.basic)
                    o.name += ran.chooseBotName()
                    o.color = 12
                    ...
                    o.skill.set(Array.from({ length: 10 }, () => Math.floor(Math.random() * 10)))
                    o.team = -3
                    o.controllers.push(new io_wanderT3(o))
                    o.refreshBodyAttributes()
                    tanks3.push(o)
                  }
                
                  tanks3 = tanks3.filter(e => { return !e.isDead(); });
                  tanks3.forEach(o => {
                    if (o.skill.level < 45) {
                      o.skill.maintain();
                    }
          ...
          if (tanks4.length < c.TANKST4) {
                    let o = new Entity(room.randomType("bas4"))
                    o.define(Class.bot)
                    o.define(Class.basic)
                    o.name += ran.chooseBotName()
                    o.color = 15
                    ...
                    o.skill.set(Array.from({ length: 10 }, () => Math.floor(Math.random() * 10)))
                    o.team = -4
                    o.controllers.push(new io_wanderT4(o))
                    o.refreshBodyAttributes()
                    tanks4.push(o)
                  }
                
                  tanks4 = tanks4.filter(e => { return !e.isDead(); });
                  tanks4.forEach(o => {
                    if (o.skill.level < 45) {
                      o.skill.maintain();
                    }
